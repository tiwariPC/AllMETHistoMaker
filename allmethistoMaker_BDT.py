...
CRSRFiles = [CRSRPath+'/' +fl for fl in os.listdir(CRSRPath) if ('_'+var_bdt in fl and 'CR' in fl) or ('_'+var_bdt in fl and 'SR' in fl)]
...
for infile in CRSRFiles:
    ...
    if 'Up.root' in infile or 'Down.root' in infile:
        ...
        if '_'+var_bdt+'_' in laststr:
            syst = laststr.partition(var_bdt)[-1].replace('.root', '')
    ...
    for infile in SignalFiles:
        ...
        for syst in [var_bdt,'CMSyear_eff_b','CMSyear_fake_b','EWK','CMSyear_Top','CMSyear_trig_met','CMSyear_trig_ele', 'CMSyear_EleID', 'CMSyear_EleRECO', 'CMSyear_MuID','CMSyear_MuISO', 'CMSyear_MuTRK','CMSyear_PU','En','CMSyear_mu_scale','CMSyear_pdf','CMSyear_prefire','JECAbsolute','JECAbsolute_year','JECBBEC1','JECBBEC1_year','JECEC2','JECEC2_year','JECFlavorQCD','JECHF','JECHF_year','JECRelativeBal','JECRelativeSample_year'] :
            if syst == var_bdt:
                ...
                for ud in ['Up','Down']:
                    temp = setHistBin(fin.Get('h_reg_SR_'+cat+'_'+var_bdt+'_'+syst+ud), 'h_reg_SR_'+cat+'_'+var_bdt+'_'+syst+ud)
                    if  temp.Integral() == 0.0:
                        for bin in range(1,temp.GetXaxis().GetNbins()+1):
                            temp.SetBinContent(bin,0.00001)
                            if temp.GetBinError(bin)<0:
